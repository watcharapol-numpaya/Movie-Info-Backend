...
const addFavoriteMovie = `UPDATE users
SET favorite_movie = (
    SELECT array_agg(DISTINCT m)
    FROM unnest(array_cat(favorite_movie, $1::int[])) AS m
    WHERE m NOT IN (SELECT unnest(favorite_movie) FROM users WHERE userId = $2)
)
WHERE userId = $2
RETURNING *
`;
